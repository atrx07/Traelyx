...
- M2.8 fixture acceptance preserves Android sensor status and raw device-frame values without compensation. For the Tecno LH8n fixture, persistent accelerometer status `0` alone is non-blocking because independent six-orientation and Physics Toolbox checks found stable, plausible response with a repeatable roughly +0.04 g positive Z-axis bias. Acceptance still requires continuous, ordered, finite/plausible accelerometer and gyroscope channels, truthful quality propagation, positive GNSS readiness before departure, locked-screen continuity, finalization/recovery, archive integrity, and complete timeline coverage. Calibration/confidence semantics belong to M3.
...
- 2026-08-13: The clean disposable Tecno rehearsal passed Start, locked/background capture, Stop/finalization, exact archive/hash verification, and isolated cleanup. The first formal motorcycle attempt then finalized and indexed 41m08.2s of intact pocket-carried evidence: 3,615 chunks, 1,788 GNSS, 427,510 accelerometer, and 495,956 gyroscope samples, with no checksum, sequence, corruption, or adjacent-stream discontinuity failure. The maintainer observed battery moving from 40% to 38%; this coarse two-point change is not a controlled drain measurement.
- 2026-08-13: Post-drive inspection found that GNSS began only 11m20.2s after recording start, then remained continuous at approximately 1 Hz. The previous inspectors reported only adjacent sample gaps and therefore hid this leading coverage loss behind a 1.0s result. Native and independent inspectors now include leading/trailing channel coverage, archive bounds are cross-checked, active status is polled, and Drive remains in an explicit first-fix state until GPS plus both motion streams are observed. Android reported unreliable accuracy for every accelerometer event while gyroscope and accepted GNSS quality counters remained clean; the raw flag is preserved and the Drive UI now propagates persistent—not merely transient—motion-confidence limitation. This attempt corroborates the maintainer's approximate stops/speed phases but is diagnostic only: pocket/body-relative IMU is not vehicle-frame evidence, annotations are not ground truth, and M2.8 remains incomplete pending a first-fix-confirmed repeat.
