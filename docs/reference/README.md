...
`CLOUD_SCHEMA_V1.md` describes the optional M6.1 Supabase foundation fields and
access boundary; the SQL migration is its executable source of truth.

`AUTH_SETUP.md` records the M6.2 optional account build configuration,
redirect allowlist, and physical validation requirements.
...
- `explanation-v1.yaml` is the implemented deterministic M4.7 reason-path contract. It maps every governed event, integrity state, score dimension/contribution, overall score, Drive DNA dimension, lifecycle state, and source version into typed presentation-ready data while keeping localized copy, Flutter UI, persistence, commentary, and any semantic recomputation out of scope.
