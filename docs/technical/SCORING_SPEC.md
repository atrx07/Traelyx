...
Drive DNA version 1 consumes compact scoring-version-1 observations without mutating the original `TripScoreAudit`. Scoring-v1 continues to report its per-trip consistency slot as unavailable and continues to exclude consistency from overall synthesis. The separately versioned Drive DNA profile derives consistency across at least three available direct profile dimensions in a caller-supplied comparable cohort. This preserves scoring-v1 historical meaning while allowing M4.5 to add a multidimensional cross-trip profile. Exact aggregation and evidence rules live in `DRIVE_DNA_SPEC.md` and `docs/reference/drive-dna-v1.yaml`; personal/vehicle lifecycle and automatic cohort selection remain M4.6.
