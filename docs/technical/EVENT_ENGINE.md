...
## 16. M5.8 result-presentation boundary

The local trip-result adapter may project an already-persisted finite normalized event severity as a `0–100` relative magnitude only when its persisted calibration-version token is valid. The UI must identify the calibration, avoid named severity bands, and state that magnitude is not a safety, moral, or legal judgment. This is presentation of existing evidence, not a new event calculation or historical recomputation.

Phone-movement events continue to suppress numeric magnitude even if the schema row contains a placeholder normalized severity, because version 1 has no calibrated per-event movement magnitude. Persisted event confidence is validated but its numeric percentage is not presented; M5.8 has no governed display calibration for that value. Unknown event identifiers fail closed to a generic recorded-event label without echoing the stored identifier.
